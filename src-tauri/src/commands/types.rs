...
pub struct Settings {
    ...
    #[serde(
        rename = "maxDownloadRetries",
        default = "default_max_download_retries"
    )]
    pub max_download_retries: u32,
    #[serde(
        rename = "dependencyMissingBehavior",
        default = "default_dependency_missing_behavior"
    )]
...
pub struct WorkshopSourceSettings {
    ...
    #[serde(
        rename = "dependencySdkRefresh",
        default = "default_dependency_sdk_refresh"
    )]
    pub dependency_sdk_refresh: String,
    #[serde(
        rename = "dependencyHtmlRefresh",
        default = "default_dependency_html_refresh"
    )]
...
#[cfg(test)]
mod tests {
    use super::{is_dummy_addon_info, Settings};
    use serde_json::json;
    ...
    #[test]
    fn recognizes_new_and_legacy_dummy_markers() {
        assert!(is_dummy_addon_info(&json!({ "l4adummy": "1" })));
        assert!(is_dummy_addon_info(&json!({
            "addonDescription": "A dummy addon generated by Left 4 Addons"
        })));
        assert!(!is_dummy_addon_info(&json!({ "l4adummy": "0" })));
    }
}
...
pub struct Addon {
    ...
    #[serde(rename = "workshopIdCandidate", default)]
    pub workshop_id_candidate: Option<String>,
    #[serde(rename = "workshopIdSource", default)]
    pub workshop_id_source: Option<String>,
    #[serde(rename = "workshopIdValidationStatus", default)]
    pub workshop_id_validation_status: Option<String>,
    #[serde(rename = "workshopIdLastAttemptAt", default)]
    pub workshop_id_last_attempt_at: Option<String>,
...
pub(crate) fn is_dummy_addon_info(addon_info: &serde_json::Value) -> bool {
    let has_marker = addon_info
        .get("l4adummy")
        .or_else(|| addon_info.get("l4aDummy"))
        .and_then(|v| v.as_str())
        .map(|v| v.trim() == "1")
        .unwrap_or(false);
    let has_legacy_description = addon_info
    ...
    has_marker || has_legacy_description
}
