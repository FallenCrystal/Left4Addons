...
use super::types::{
    is_dummy_addon_info, Addon, Database, Group, KnownAddonEntry, LocalDatabase,
    MasterCollection, RenameItem, Settings, WorkshopSeenItem,
};
...
pub async fn move_addons(
    ...
    if target_dir_type != "loading" && target_dir_type != "workshop" {
        return Err(format!("Invalid target directory type: {}", target_dir_type));
    }
    ...
    for id in ids {
        ...
        if dest_path.exists() && !(target_dir_type == "workshop" && is_dummy_vpk(&dest_path)) {
            errors.push(format!("Target file already exists: {}", dest_path.display()));
            continue;
        }
    ...
    for plan in plans {
        ...
        if target_dir_type == "loading" && plan.from_dir_type == "workshop" && db.settings.enable_dummy_bypass {
            if let Some(ref w_id) = plan.workshop_id {
                ...
                generate_dummy_vpk(&plan.dest_path, &dummy_vpk_path)
                    .map_err(|e| format!("Failed to generate dummy addon for {}: {}", plan.id, e))?;
            }
        }
    }
...
pub async fn rename_addons(
    ...
    for plan in plans {
        ...
        let old_base = Path::new(&plan.old_vpk_name).file_stem().and_then(|s| s.to_str()).unwrap_or("").replace(".disabled", "");
        let new_base = Path::new(&plan.sanitized).file_stem().and_then(|s| s.to_str()).unwrap_or("");
            ...
            if let Some(addon) = db.addons.get(id) {
                ...
                if disabled_path.exists() {
                    fs::remove_file(&disabled_path)
                        .map_err(|e| format!("Failed to delete {}: {}", disabled_path.display(), e))?;
                }
            }
        }
    }
...
pub async fn download_addon(
    ...
    if dest_path.exists() {
        return Err(format!("Addon file already exists: {}", dest_path.display()));
    }
    let disabled_dest_path = dest_path.with_extension("vpk.disabled");
    if disabled_dest_path.exists() {
        return Err(format!("Disabled addon file already exists: {}", disabled_dest_path.display()));
    }
    ...
    if !response.status().is_success() {
        return Err(format!("Download server responded with status {}", response.status()));
    }

    let total_size = response.content_length().unwrap_or(0);
    let mut file = fs::File::create(&tmp_path).map_err(|e| format!("Failed to create local file: {}", e))?;
    ...
    file.flush().map_err(|e| format!("Failed to flush downloaded file: {}", e))?;
    drop(file);
