...
- 只要 L4A 调用过一次 SDK, Steam 就会一直显示你“正在玩求生之路”, 直到 L4A 进程完全结束.   
  如果你在 Steam 库里好奇点了一下“停止”，被强制关掉的会是 L4A 本身.
- 只要 Left 4 Addons 调用一次 SDK, Steam 会显示你正在玩求生之路, 直到你退出 L4A.
...
你别管它有不有趣 我就是不知道放哪里了而已

- Watt Toolkit (Steam++) 是我目前唯一已知的免费加速器,   
  且所有已知的Steam域名都会经过其提供的公益 CDN.  
  它也可以加速 L4A 和几乎满速的创意工坊下载.
- L4A 调用 SDK 后, 你可以在另一台计算机串游 L4A 本体.  
  但如果Tauri拉起的 WebView进程与 L4A 独立, 则会持续显示黑屏.
- UU 加速器的加速 Steam 是基于进程的. 可能与你想象的正好相反.
- 某些加速器虽然提供低延迟体验, 但通常有速率限制.  
  很有可能你不挂加速器在可以直连的情况下  
  下载的速度的比挂着加速器要快得多
- UU加速器加速求生之路只加速联机, 不加速创意工坊

至于为什么只有这两个加速器, 因为我只用过这两个.
